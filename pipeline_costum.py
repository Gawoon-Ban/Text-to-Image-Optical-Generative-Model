...
class DDPMPipeline_TextCond(DiffusionPipeline):
    r"""
    Custom DDPM Pipeline for Text-Conditioned Image Generation.
    Parameters:
        unet: UNet2DModel
    ...
    def __call__(
        self,
        batch_size: int = 1,
        text_emb: Optional[torch.Tensor] = None,    # shape: (batch_size, embed_dim)
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        num_inference_steps: int = 1000,
        output_type: Optional[str] = "pil",
        return_dict: bool = True,
        ...
        """
        Args:
            batch_size: 이미지 생성 개수
            text_emb: 텍스트 임베딩 (예: CLIP 등에서 획득), shape: (batch_size, embed_dim)
            generator: torch random generator
            num_inference_steps: 디퓨전 스텝 개수
        ...
        # FrozenDict key-style config 접근
        config = unet.config
        if isinstance(config["sample_size"], int):
            ...
            image_shape = (batch_size, config["in_channel"], *config["sample_size"])
        ...
        # Check text_emb
        if text_emb is None:
            raise ValueError("text_emb (text embedding) must be provided for text-to-image sampling.")
        if text_emb.shape[0] != batch_size:
            raise ValueError(f"text_emb batch size ({text_emb.shape[0]}) does not match batch_size ({batch_size})")
        text_emb = text_emb.to(self.device)

        # DDPM Sampling Loop
        for t in self.progress_bar(self.scheduler.timesteps):
            # UNet에서 text_emb 조건을 함께 전달
            # UNet forward: (image, timestep, text_emb)
            model_output = unet(image, t, text_emb, return_dict=False)[0]
            image = self.scheduler.step(model_output, t, image, generator=generator).prev_sample
        ...
        if not return_dict:
            if output_type == "tensor":
                return (image, noise_init, text_emb)
            else:
                return (image,)
        return ImagePipelineOutput(images=image)